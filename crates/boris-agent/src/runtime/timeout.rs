...
/// Await `tool.execute(args)`, aborting the wait after `timeout`.
///
...
mod tests {
    ...
    async fn timeout_fires() {
        let err = run_with_timeout(&SlowTool, json!({}), Duration::from_millis(80))
            .await
            .unwrap_err();
    ...
    #[tokio::test]
    async fn fast_ok() {
        let out = run_with_timeout(&FastTool, json!({}), Duration::from_secs(1))
            .await
            .unwrap();
        assert_eq!(out, "ok");
    }
