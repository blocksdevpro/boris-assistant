...
use crate::tools::web::parse_safe_http_url;
...
impl Tool for OpenUrlTool {
    ...
    fn description(&self) -> &str {
        "Open a public http(s) URL in the user's default browser. Requires user confirmation. \
         Same host policy as web_fetch: localhost, private, link-local, and metadata hosts are denied."
    }
    ...
    fn parameters(&self) -> Value {
        json!({
            ...
            "properties": {
                "url": {
                    "type": "string",
                    "description": "Full public http or https URL to open (private/local hosts denied)"
                }
            },
            "required": ["url"]
        })
