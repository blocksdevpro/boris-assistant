...
//!
//! This test has real side effects (clipboard writes, a local shell command,
//! network access, and an OS open request). It is ignored by default and also
//! requires explicit consent to run:
//! `BORIS_RUN_LIVE_TOOL_SMOKE=1 cargo test -p boris-agent --test tool_live_smoke -- --ignored --nocapture`
...
use boris_agent::tools::files::{EditFileTool, FsRoots, ListDirTool, ReadFileTool, WriteFileTool};
...
use boris_agent::tools::profile::{GetUserContextTool, SaveUserFactTool, UpdateUserProfileTool};
...
#[tokio::test]
#[ignore = "live smoke performs clipboard, shell, network, and OS-open side effects"]
async fn live_smoke_all_boris_tools() {
    assert_eq!(
        std::env::var("BORIS_RUN_LIVE_TOOL_SMOKE").as_deref(),
        Ok("1"),
        "set BORIS_RUN_LIVE_TOOL_SMOKE=1 before running this live, side-effecting smoke test"
    );

    let root = std::env::temp_dir().join(format!("boris-tool-smoke-{}", std::process::id()));
    ...
    results.push(
        run_one("remember_note", async {
            RememberNoteTool::new(notes_path.clone())
                .execute(
                    &boris_agent::ToolCallContext::new("t"),
                    json!({"note": "smoke test note from tool_live_smoke"}),
                )
                .await
                .map_err(|e| e.message)
        })
    ...
    results.push(
        run_one("save_user_fact", async {
            SaveUserFactTool::with_path(profile.clone(), profile_path.clone())
                .execute(
                    &boris_agent::ToolCallContext::new("t"),
                    json!({"fact": "likes smoke tests", "category": "preference"}),
                )
                .await
                .map_err(|e| e.message)
        })
    ...
    results.push(
        run_one("update_user_profile", async {
            UpdateUserProfileTool::with_path(profile.clone(), profile_path.clone())
                .execute(
                    &boris_agent::ToolCallContext::new("t"),
                    json!({"preferred_name": "Smoke Tester", "address_as": "Tester"}),
                )
                .await
                .map_err(|e| e.message)
        })
    ...
    results.push(
        run_one("clipboard_set", async {
            ClipboardSetTool
                .execute(
                    &boris_agent::ToolCallContext::new("t"),
                    json!({"text": "boris-smoke-clipboard"}),
                )
                .await
                .map_err(|e| e.message)
        })
