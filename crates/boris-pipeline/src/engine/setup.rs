...
use super::models::{
    create_stt, create_stt_loader, create_tts, create_tts_loader, ModelLoader, SttBox, TtsBox,
};
use super::picture::Picture;
use super::util::{env_flag_false, session_token};
use super::MIC_QUEUE;
...
pub(super) struct EngineRuntime {
    ...
    pub stt: SttBox,
    pub tts: TtsBox,
    pub stt_loader: ModelLoader<SttBox>,
    pub tts_loader: ModelLoader<TtsBox>,
...
pub(super) fn init_runtime(
    ...
    tracing::info!(
        ...
        vad_bytes = config.vad_model.len(),
    ...
    // Reused for the engine lifetime; turns transfer model ownership through
    // these workers instead of spawning two new OS threads every turn.
    let stt_loader = create_stt_loader();
    let tts_loader = create_tts_loader();
    ...
    let vad = load_vad(&config, &status_tx)?;
    ...
    let maintenance = boris_agent::MaintenanceWorker::spawn();
    let mut agent = build_agent(&config);
    agent.set_maintenance(maintenance.handle());
...
fn load_vad(
    ...
    let threshold = vad_threshold_from_env();
    tracing::info!(
        vad_bytes = config.vad_model.len(),
        threshold,
        "loading SileroVad (ORT session)…"
    );
    let started = std::time::Instant::now();
    match SileroVad::try_new_with_threshold(&config.vad_model, threshold) {
        Ok(v) => {
            tracing::info!(
                ms = started.elapsed().as_millis() as u64,
        ...
        Err(e) => {
            tracing::error!(
                error = %e,
                vad_bytes = config.vad_model.len(),
                "SileroVad::try_new FAILED — often missing onnxruntime.dll / DirectML.dll beside the exe"
            );
            crate::diagnostics::log_environment("silero_vad_init_failed");
            let detail = format!("silero vad init: {e}");
            fault(status_tx, config, detail.clone());
            Err(PipelineError::init(detail))
        }
    }
...
fn vad_threshold_from_env() -> f32 {
    match std::env::var("BORIS_VAD_THRESHOLD") {
        Ok(raw) => match raw.trim().parse::<f32>() {
            Ok(t) if t.is_finite() && t > 0.0 && t <= 1.0 => t,
            _ => {
                tracing::warn!(
                    value = %raw,
                    "invalid BORIS_VAD_THRESHOLD; using default"
                );
                SILERO_SPEECH_THRESHOLD
            }
        },
        Err(_) => SILERO_SPEECH_THRESHOLD,
    }
}
